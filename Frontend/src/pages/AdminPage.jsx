        ...
        <section style={s.card}>
          ...
          <div style={s.archGrid}>
            {ARCH_LAYERS.map(layer => (
              <div key={layer.name} style={s.archItem}>
                <span style={s.archIcon}>{layer.icon}</span>
                <div>
                  <p style={s.archName}>{layer.name}</p>
                  <p style={s.archDesc}>{layer.desc}</p>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '6px 0',
      ...
      {link
        ...
        : <span style={{ fontSize: 13, fontFamily: mono ? 'monospace' : 'inherit',
...
const s = {
  page:      { minHeight: '100vh', background: BRAND.surface, color: BRAND.text,
               fontFamily: "'DM Sans', sans-serif" },
  header:    { padding: '24px 32px', borderBottom: `1px solid ${BRAND.border}`,
               background: 'rgba(10,15,30,0.95)' },
  brand:     { display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 },
  dot:       { width: 10, height: 10, borderRadius: '50%', background: BRAND.accent,
               boxShadow: `0 0 12px ${BRAND.accent}` },
  brandName: { fontWeight: 800, fontSize: 20, letterSpacing: '-0.03em' },
  badge:     { fontSize: 11, fontWeight: 700, background: `${BRAND.primary}40`,
  ...
  th:        { textAlign: 'left', padding: '8px 12px', color: BRAND.muted,
               fontSize: 11, textTransform: 'uppercase', letterSpacing: '0.06em',
               borderBottom: `1px solid ${BRAND.border}` },
  tr:        { borderBottom: `1px solid ${BRAND.border}` },
  td:        { padding: '10px 12px' },
  mono:      { fontFamily: 'monospace', fontSize: 12 },
  stagePill: { fontSize: 11, fontWeight: 700, padding: '3px 10px',
               borderRadius: 20, color: '#fff', letterSpacing: '0.04em' },
  smallBtn:  { fontSize: 11, padding: '4px 12px', borderRadius: 6,
  ...
  archItem:  { display: 'flex', alignItems: 'flex-start', gap: 12, padding: '10px',
